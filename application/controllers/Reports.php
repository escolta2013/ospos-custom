...
class Reports extends Secure_Controller
{
	public function __construct()
	{
		...
		//$this->load->model('reports/Summary_accounts_payable_by_category');
 ...
 function summary_accounts_payable_by_category($start_date, $end_date, $sale_type)
{
    // 1) Permiso
   //if ( ! $this->Employee->has_grant('reports_accounts_payable_by_category', $this->session->userdata('person_id')))
    //{
    //    show_error($this->lang->line('reports_no_permission'));
    //}

    // 2) Inputs vía URL (ningún get_post aquí)
    $inputs = array(
    ...
    // 3) Cargo el modelo
    $this->load->model('reports/Summary_accounts_payable_by_category');
    $model = $this->Summary_accounts_payable_by_category;

    // 4) Obtengo datos y totales
    $report_data = $model->getData($inputs);
    $summary     = $this->xss_clean($model->getSummaryData($inputs));

    // 5) Formateo para la tabla
    $tabular_data = array();
    foreach($report_data as $row)
    {
        $tabular_data[] = $this->xss_clean(array(
            'category_name'  => $row['category_name'],
            'total_invoiced' => to_currency_no_money($row['total_invoiced']),
            'total_due'      => to_currency_no_money($row['total_due']),
            'next_due_date'  => date($this->config->item('dateformat'), strtotime($row['next_due_date']))
        ));
    }

    // 6) Preparo payload para la vista tabular
    $data = array(
        'title'        => $this->lang->line('reports_accounts_payable_by_category'),
    ...
    // 7) Renderizo con la misma vista que el resto de resúmenes
    $this->load->view('reports/tabular', $data);
}
	...
	public function summary_taxes($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = $this->xss_clean(array(
				'tax_name' => $row['name'],
				'tasa_tax' => 'Bs '.to_currency_no_money($row['tasa_tax']) ,
				'tax_percent' => $row['percent'],
				'report_count' => $row['count'],
				'subtotal' => to_currency($row['subtotal']).' | Bs '.to_currency_no_money($row['subtotal']*$row['tasa_tax']) ,
				'tax' => to_currency_tax($row['tax']).' | Bs '.to_currency_no_money($row['tax']*$row['tasa_tax']),
				'total' => to_currency($row['subtotal']+$row['tax']).' | Bs '.to_currency_no_money(($row['subtotal']+$row['tax'])*$row['tasa_tax'])
			));
		}
	...
	public function summary_payments($start_date, $end_date)
	{
		...
		foreach($report_data as $row)
		{
			...
			else
			{
               ...
               if ($row['trans_type']=='CashBs' || $row['trans_type']=='PGM' || $row['trans_type']=='Debit') {
               	$tabular_data[] = $this->xss_clean(array(
					'trans_group' => $row['trans_group'],
					'trans_type' => $row['trans_type'],
					'trans_sales' => $row['trans_sales'],
					'trans_amount' => to_currency($row['trans_amount']),
					'trans_payments' => to_currency($row['trans_payments']).' x Tasa Bs '.$row['T1'].' = '.to_currency($row['trans_payments']*str_replace(",", ".", $this->config->item('tasa'))),
					'trans_refunded' => to_currency($row['trans_refunded']).' x Bs '.$row['T1'].' = '.to_currency($row['trans_refunded']*str_replace(",", ".", $this->config->item('tasa'))),
					'trans_due' => to_currency($row['trans_due']).' x Bs '.$row['T1'].' = '.to_currency($row['trans_due']*str_replace(",", ".", $this->config->item('tasa')))
				));
