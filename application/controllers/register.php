...
if ($this->config->item('auto_cambio_tasa')==1) {
	   ...
	   $url = 'https://pydolarvenezuela-api.vercel.app/api/v1/dollar/';
		...
		if ($response) {
			...
			$formatted_value_bcv =  $objeto['bcv']['price'];
			$formatted_value_paralelo =  $objeto['enparalelovzla']['price'];				  
			}		 
...
?> -->
 <!--   <?php if($cambiotasa==1 && $this->config->item('tasa')!=$tasa) { ?>   
     <div align="center" class="alert alert-warning" role="alert">
       <span class="closebtn">&times;</span> 
         <?php if ($this->config->item('tasa_ref')==1) { ?>
           <div class="module_item">  
            <h5 class="title-prome"><b>Atención La Tasa De Cambio BCV (Oficial) Es Diferente a la del Sistema!</b></h5>
            <h6> <a href="<?php echo site_url('/sales/cambiar_tasa');?>" onclick="return true" style="text-align:center; color: #337ab7;font-size: 14px;"> Haga Click Para Actualizar </a></h6>



            <img src="<?php echo base_url('images/BCV.png'); ?>" title="logo-bcv" alt="logo-bcv" width="50" height="50">
              
             <h5><b><?php echo"Tasa BCV Bs.: ",  $tasa; ?></b></h5>  
             <h5><b><?php echo"Tasa Sistema Bs.: ",  $this->config->item('tasa'); ?></b></h5>
          </div>
        <?php   }else if ($this->config->item('tasa_ref')==2) { ?>

          <div class="module_item">  
            <h5 class="title-prome"><b>Atención La Tasa De Cambio @EnParaleloVzla3 Es Diferente a la del Sistema!</b></h5>
             <h6> <a href="<?php echo site_url('/sales/cambiar_tasa');?>" onclick="return true" style="text-align:center; color: #337ab7;font-size: 14px;"> Haga Click Para Actualizar </a></h6>
            <img src="<?php echo base_url('images/logo_paralelo.png'); ?>" title="logo-bcv" alt="logo-bcv" width="50" height="50">
            <h5><b><?php echo"Tasa @EnParaleloVzla3 Bs.: ",  $tasa; ?></b></h5> 
            <h5><b><?php echo"Tasa Sistema Bs.: ",  $this->config->item('tasa'); ?></b></h5> 
          </div>
					...
					<?php echo form_open($controller_name."/edit_item/$line", array('class'=>'form-horizontal', 'id'=>'cart_'.$line)); ?>
						<tr>
							...
							<td>
								...
								{
									echo form_input(array('name'=>'price', 'class'=>'form-control input-sm', 'value'=>to_currency_no_money($item['price']), 'tabindex'=>++$tabindex, 'onClick'=>'this.select();')).'<b>Bs: </b>'.$item['price']*str_replace(",", ".", $this->config->item('tasa'));
								...
								else
								{
									...
									echo form_hidden('price', to_currency_no_money($item['price'])).'<br><b>Bs: </b>'.$item['price']*str_replace(",", ".", $this->config->item('tasa'));
								}
								?>
							</td>
							...
							<td>
								...
								{ 
									// echo to_currency($item['discounted_total']).'<br><b>Bs: </b>'.$item['discounted_total']*$item['price']*str_replace(",", ".", $this->config->item('tasa'));

									echo to_currency($item['discounted_total']).'<br><b>Bs: </b>'.$item['discounted_total']*str_replace(",", ".", $this->config->item('tasa'));
								}
								?>
							</td>
...
<div id="overall_sale" class="panel panel-default">
	<div class="panel-body">
			...
			?>
				<table class="sales_table_100">
					<tr>
						...
						<!-- Nukerj 14-01-2023 se corrigio que no se mostraba el nombre del cliente en la información -->
						<th style="width: 45%; text-align: right;"><?php echo anchor('customers/view/'.$customer_id, $first_name.' '.$last_name, array('class' => 'modal-dlg', 'data-btn-submit' => $this->lang->line('common_submit'), 'title' => $this->lang->line('customers_update'))); ?></th>
					</tr>

					<?php
					if($cedula!=0) // Nukerj 14-01-2023
					{
					?>
						<tr>
							<th style="width: 55%;"><?php echo $this->lang->line("common_cedula"); ?></th>
							<th style="width: 45%; text-align: right;"><?php echo $cedula; ?></th>
						</tr>
					<?php
					}
					?>
		...
		<table class="sales_table_100" id="sale_totals">
			...
			<tr>
				<th style="width: 55%;"><?php echo $this->lang->line('sales_sub_total').' '.$this->config->item('currency_symbol'); ?></th>
				<th style="width: 45%; text-align: right;"><?php echo to_currency($subtotal); ?></th>
			</tr>

			<tr>
				<th style="width: 55%;"><?php echo "Bs"; ?></th>
				<th style="width: 45%; text-align: right;"><?php echo to_currency_no_money($subtotal*str_replace(",", ".", $this->config->item('tasa'))); ?></th>
			</tr>
			...
			?>
				<tr>
					<th style="width: 55%;"><?php echo (float)$tax['tax_rate'] . '% ' . $tax['tax_group']; ?>
						
						<?php
                         if ($tax['tax_group']=="IGTF") {
                         	 if($this->sale_lib->get_igtf()==1){
                     	?>
                     	  <a href="<?php echo site_url('/sales/desactivar_igtf');?>" onclick="return true" style="text-align:center;border-bottom: dashed 1px #08c;color: #337ab7;font-size: 14px;"> Desactivar
                                         </a>  <br> 
                     	<?php
                        } 
                     } else {
                     	 if($this->sale_lib->get_iva()==1){
                     	?>

                     	 <a href="<?php echo site_url('/sales/desactivar_iva');?>" onclick="return true" style="text-align:center;border-bottom: dashed 1px #08c;color: #337ab7;font-size: 14px;"> Desactivar
                                         </a>  <br> 
                     	<?php
                       } 
                     }
                       
                       if($this->sale_lib->get_iva()==0){
                       	?>
                       	 <a href="<?php echo site_url('/sales/activar_iva');?>" onclick="return true" style="text-align:center;border-bottom: dashed 1px #08c;color: #337ab7;font-size: 14px;"> Activar Iva </a> 
                                         <?php
                       }
                       
                       if($this->sale_lib->get_igtf()==0){
                        	?><a href="<?php echo site_url('/sales/activar_igtf');?>" onclick="return true" style="text-align:center;border-bottom: dashed 1px #08c;color: #337ab7;font-size: 14px;"> Activar IGTF
                                         </a><?php  
                        }
						?>
					</th>
					<th style="width: 45%; text-align: right;"><?php echo  $tax['sale_tax_amount'].' | Bs '.$tax['sale_tax_amount']*str_replace(",", ".", $this->config->item('tasa')); ?></th>
					
				</tr>
			<?php
			}

			if ($this->sale_lib->get_iva()==0 && $this->sale_lib->get_igtf()==0) {
				?><a href="<?php echo site_url('/sales/reactivar_impuestos');?>" onclick="return true" style="text-align:center;border-bottom: dashed 1px #08c;color: #337ab7;font-size: 14px;"> Reactivar Impuestos
                                         </a><br><H4>NOTA DE ENTREGA</H4><?php
			}
			?>

			<tr>
				<th style="width: 55%; font-size: 150%"><?php echo $this->lang->line('sales_total').' '.$this->config->item('currency_symbol'); ?></th>
				<th style="width: 45%; font-size: 150%; text-align: right;"><span id="sale_total"><?php echo to_currency($total); ?></span></th>
			</tr>

			 <tr>
			 <tr> <!-- Nukerj 14-01-2023 Mostrar total Bs -->	
			 	<th style="width: 55%; font-size: 150%"> </th>
			 	<th style="width: 45%; font-size: 150%; text-align: right;"><span id="sale_total"><?php echo"Bs ", to_currency_no_money($total*str_replace(",", ".", $this->config->item('tasa'))); ?></span></th>
			 </tr>
		...
		?>
			<table class="sales_table_100" id="payment_totals">
				<tr>
					...
					<th style="width: 45%; text-align: right;"><?php echo to_currency($payments_total).' | Bs '.to_currency_no_money($payments_total*str_replace(",", ".", $this->config->item('tasa'))); ?></th>
				...
				<tr>
					<th style="width: 55%; font-size: 120%"> </th>
					<th style="width: 45%; font-size: 120%; text-align: right;"><span id="sale_amount_due"><?php echo"Bs ", to_currency_no_money ($amount_due*str_replace(",", ".", $this->config->item('tasa'))); ?></span></th>
				</tr>
			</table>
			...
			<div id="payment_details">
				...
				<table class="sales_table_100">
					<tr>							 
						<?php echo form_open($controller_name."/change_method", array('id'=>'mode_method', 'class'=>'form-horizontal panel panel-default')); ?>		 

						<td><?php echo "Metodo De Pago"; ?></td>
						<td>
							<?php echo form_dropdown('payment_method', $payment_methods, $payment_method, array('onchange'=>"$('#mode_method').submit();", 'class'=>'selectpicker show-menu-arrow', 'data-style'=>'btn-default btn-sm', 'data-width'=>'fit')); ?>											
						</td>
					...
					<table class="sales_table_100" id="register">
						<thead>
							<tr>
								...
								<th style="width: 40%;"><?php echo $this->lang->line('sales_payment_type'); ?></th>
								<th style="width: 50%;"><?php echo $this->lang->line('sales_payment_amount'); ?></th>
							</tr>
						</thead>
							...
							?>
								<tr>
									...
									if ($payment['payment_type']=='CashBs' || $payment['payment_type']=='PGM' || $payment['payment_type']=='Debit') {
										 echo  to_currency_no_money($payment['payment_amount']*str_replace(",", ".", $this->config->item('tasa'))).' = '. to_currency($payment['payment_amount']);
										
									} else {
										 echo to_currency($payment['payment_amount'])."= Bs ".to_currency_no_money($payment['payment_amount']*str_replace(",", ".", $this->config->item('tasa')));
									} 
									    ?></td>
								</tr>
							<?php
...
function check_payment_type()
{
	...
	// Nukerj 14-01-2023
	else if(($("#payment_types").val() == 'CashBs') || $("#payment_types").val()=='PGM'  || $("#payment_types").val()=='Debit')
	{
		...
		$("#amount_tendered:enabled").val("<?php echo to_currency_no_money($cash_amount_due*str_replace(",", ".", $this->config->item('tasa'))); ?>");
