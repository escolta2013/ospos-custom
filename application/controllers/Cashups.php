...
class Cashups extends Secure_Controller
{
	...
	*/
	public function ajax_cashup_total()
	{
		...
		$total = $this->_calculate_total($open_amount_cash, $transfer_amount_cash, $closed_amount_cash, $closed_amount_due, $closed_amount_card, $closed_amount_check);
	...
	Calculate total
	*/
	private function _calculate_total($open_amount_cash, $transfer_amount_cash, $closed_amount_cash, $closed_amount_due, $closed_amount_card, $closed_amount_check)
	{
		// Asegurarse de que todos los valores sean numéricos
		$open_amount_cash = floatval($open_amount_cash);
		$transfer_amount_cash = floatval($transfer_amount_cash);
		$closed_amount_cash = floatval($closed_amount_cash);
		$closed_amount_due = floatval($closed_amount_due);
		$closed_amount_card = floatval($closed_amount_card);
		$closed_amount_check = floatval($closed_amount_check);
