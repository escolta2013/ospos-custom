...
class Sales extends Secure_Controller
{
	...
	public function manage()
	{
		...
		else
		{
			...
			$data['stock_location'] = $this->xss_clean($this->sale_lib->get_sale_location());
	...
	public function search()
	{
        ...
        $this->sale_lib->set_sale_location($this->input->get('location_id'));
	...
	public function desactivar_iva()
	{   
		$act_iva = 0;		  
		$this->sale_lib->set_iva($act_iva); 
		$this->_reload();
	}

	public function activar_iva()
	{
		$act_iva = 1;		 
		$this->sale_lib->set_iva($act_iva); 
		$this->_reload();
	}
	public function reactivar_impuestos()
	{
		$this->sale_lib->set_iva(1);
		$this->sale_lib->set_igtf(1);  
		$this->_reload();
	}
	

	public function desactivar_igtf()
	{   
		$act_igtf = 0;		  
		$this->sale_lib->set_igtf($act_iva); 
		$this->_reload();
	}

	public function activar_igtf()
	{
		$act_igtf = 1;		 
		$this->sale_lib->set_igtf($act_igtf); 
		$this->_reload();
	}
		...
		public function change_method()
	{
		...
		$this->sale_lib->set_method($payment_method);
		$this->_reload();
	}
	...
	private function _load_customer_data($customer_id, &$data, $stats = FALSE)
	{
		...
		if($customer_id != -1)
		{
			...
			$data['cedula'] = $customer_info->cedula; // Nukerj 14-01-2023
	...
	private function _reload($data = array())
	{
		...
		$data['payment_methods'] = $payment_methods = array('Mixto' =>"Mixto Bs|$" , 'Dolares' =>"Dolares.", 'Bolivares' =>"Bolivares" );;
		$data['payment_method'] = $this->sale_lib->get_method();
	...
	public function cambiar_tasa()
	{
	   ...
	   if ($this->config->item('auto_cambio_tasa')==1) {
		...
		if ($response) {
			...
			$formatted_value_bcv =  number_format($objeto['bcv']['price'], 2, '.', ',');
				$formatted_value_paralelo =number_format($objeto['enparalelovzla']['price'], 2, '.', ',');	
	...
	public function edit($sale_id, $tocreditprint = 0, $tocreditprintinvoice = -1)
	{
		$data = array();
		...
		$data['sale_items']=$this->xss_clean($this->Sale->get_info_list_prod($sale_id)); 
		...
		{   
			$paymentarray = get_object_vars($payment);
			foreach(get_object_vars($payment) as $property => $value)
			{
				$payment->$property = $this->xss_clean($value);
			}
			if( in_array($this->lang->line('sales_due'), $paymentarray) )
			{
				$data['payment_tocredit_value'] = $paymentarray['payment_amount']; // por cuanto fue la venta de credito
				$data['payment_tocredit_paidup'] = $this->Sale->get_payment_tocredit($sale_id); // cuanto lleva pagado en total
				$data['payment_tocredit_due'] = $data['payment_tocredit_value'] - $data['payment_tocredit_paidup']; // cuanto debe entonces
				$data['payment_tocredit_label'] = $this->lang->line('sales_due'); // marco un semaforo personalizado, que hay creditos pendientes
				$data['payment_tocredit_details'] = $this->Sale->get_payments_tocredit($sale_id);
				$hastocredit = TRUE;
			}
			$data['payments'][] = $payment;
	    /**/$totalpaymentsale = $totalpaymentsale + $paymentarray['payment_amount'];/*/*/
		}
        
        $data['pagos_recibido'] =$this->Sale->get_payments_tocredit_recibed($sale_id);

        foreach ($this->Sale->get_payments_tocredit_recibed($sale_id) as $pagos_recibidos) {
           $date = date_create($pagos_recibidos['fecha_pago']);
          $data['fecha_pago_rec']=date_format($date,"d/m/Y");
        }

	    $data['prima']= $this->Sale->get_payments_prima($sale_id);
        ...
        if($tocreditprint == 1 AND $hastocredit)
		{
			$tocreditspayments = $data['payment_tocredit_details'];
			$data['barcode'] = $this->barcode_lib->generate_receipt_barcode($tocreditprintinvoice);
			$data['paid_id'] = $tocreditprintinvoice;
			foreach($tocreditspayments as $row=>$paymentadd)
			{
				if($paymentadd['payment_tocredit'] == $tocreditprintinvoice)
				{
					$payment_amount = $paymentadd['payment_amount'];
				}
			}
			$data['payment_amount'] = $payment_amount;
			$data['payment_amount_totalsale'] = $totalpaymentsale;
			$this->load->library('table');
			$this->load->view('sales/tocredit_print', $data);
		}else if($tocreditprint == 2 AND $hastocredit)
		{
			$this->load->view('sales/form_addpayment', $data);
		}
		else
		{
	...
	public function save($sale_id = -1)
	{
		...
		/**/$payment_tocredit_todue = $this->input->post('payment_tocredit_todue');
			$payment_tocredit_amount = $this->input->post('payment_tocredit_amount');
			$payment_tocredit_invoice = $this->input->post('tocredit_invoice_number');/*/*/
		...
		{
		   if($payment_tocredit_todue > 0)
			{  
				$payment_typeadd = $this->input->post('payment_type_1');
				$resulting = $this->Sale->save_payment_tocredit($sale_id, $payment_tocredit_amount, $payment_tocredit_invoice, $payment_typeadd);
	...
	public function search_cart_for_item_id($id, $array)
	{
		...
		public function crons() {
			...
			$sale_id=$this->Sale->get_sales_items_crons();
			foreach ($sale_id as $key => $row) {

		    echo "<b>Id De Venta: ", $row['sale_id'].' | Ubicación: '.$row['item_location'].'<br>';
		         
		            $add_value = array( 
						'location' =>$row['item_location']
					);
					 $this->Sale->update_location($row['sale_id'], $add_value); 
