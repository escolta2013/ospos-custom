...
class Items extends Secure_Controller
{
	...
	public function index()
	{
		...
		$data['items'] = $this->Item->get_all_by_employee_location(); // Nueva función
	...
	public function search()
	{
		...
		$filters = array(
			...
			'stock_location_id' => $this->item_lib->get_item_location(),
			 'employee_location' => $this->session->userdata('location_id'), // <-- Filtro por ubicación
			'location_id' => $is_admin ? NULL : $employee_location, // Filtro clave
	...
	public function edit($item_id = -1)
	{
    	// Verificar permisos de edición
   		$employee_location = $this->Employee->get_logged_in_employee_current_location_id();
    	$item_location = $this->Item->get_item_location($item_id);
    
    	if ($item_location != $employee_location && !$this->Employee->has_grant('admin')) {
        	$this->session->set_flashdata('error', 'No tienes permiso para editar este artículo');
        	redirect('items');
    	}
	...
	public function delete()
	{
    	...
    	// Verificar permisos para cada item
    	foreach ($items_to_delete as $item_id) {
        	$item_location = $this->Item->get_item_location($item_id);
        
        	// Si no es admin y el artículo no pertenece a su tienda, bloquear
        	if (!$is_admin && $item_location != $employee_location) {
            	echo json_encode(array(
                	'success' => FALSE,
                	'message' => $this->lang->line('items_permission_denied') . ' (Artículo ID: ' . $item_id . ')'
            ));
            return;
        }
    }
