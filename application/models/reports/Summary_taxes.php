...
class Summary_taxes extends Summary_report
{
	protected function _get_data_columns()
	{
		return array(
			...
			array('tasa_tax' => "Tasa", 'sortable' => FALSE),
	...
	public function getData(array $inputs)
	{
		...
		$query = $this->db->query("SELECT name as name, percent, tasa_tax, COUNT(DISTINCT sale_id) AS count, ROUND(SUM(subtotal), $decimals) AS subtotal, ROUND(SUM(tax), $decimals) AS tax, ROUND(SUM(total), $decimals) AS total 
			FROM (
				SELECT
					name AS name, sales_items_taxes.tasa as tasa_tax,
