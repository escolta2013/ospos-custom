...
abstract class Summary_report extends Report
{
	...
	private function __common_select(array $inputs)
	{
		...
		$sale_cost2 = 'SUM(sales_items.item_cost_price * sales_items.quantity_purchased)*tasa_tax';
		$sales_tax2 = "IFNULL(SUM(sales_items_taxes.tax)*tasa_tax, 0)";
		...
		if($this->config->item('tax_included'))
		{
			...
			$sale_total2 = "ROUND(SUM($sale_price), $decimals)*tasa_tax + $cash_adjustment";
			$sale_subtotal2 = "$sale_total - $sales_tax*tasa_tax";

		}
		else
		{
			$sale_subtotal = "ROUND(SUM($sale_price), $decimals) + $cash_adjustment";
			$sale_subtotal2 = "ROUND(SUM($sale_price), $decimals)*tasa_tax + $cash_adjustment";
	

			$sale_total = "ROUND(SUM($sale_price), $decimals) + $sales_tax + $cash_adjustment";

			$sale_total2 = "ROUND(SUM($sale_price), $decimals)+ $sales_tax + $cash_adjustment";
		}
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_items_taxes_temp') .
			...
			(
				SELECT sales_items_taxes.tasa as tasa_tax, sales_items_taxes.sale_id AS sale_id,
		...
		$this->db->select("
				IFNULL($sale_subtotal2, $sale_total) AS subtotal2,
				$sales_tax2 AS tax2,
				IFNULL($sale_total2, $sale_subtotal2)*tasa_tax  AS total2,
				$sale_cost2 AS cost2,
				(IFNULL($sale_subtotal2, $sale_total2) - $sale_cost*tasa_tax) AS profit2
		");
	}
