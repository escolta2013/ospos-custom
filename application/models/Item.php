...
{
    // --- Extracción de "talla" del término de búsqueda ---
    $talla_value = '';
    if (!empty($search)) {
        if (preg_match('/talla\s*(\S+)/i', $search, $matches)) {
            $talla_value = trim($matches[1]);
            // Remover "talla <valor>" del término de búsqueda
            $search = trim(preg_replace('/talla\s*\S+/i', '', $search));
        }
    }
    
    // --- Selección de campos dependiendo de si se cuenta o se listan ítems ---
    if ($count_only === TRUE) {
        $this->db->select('COUNT(DISTINCT items.item_id) AS count');
    } else {
        // Selección de columnas básicas de items
    ...
    // --- FROM y JOIN principales ---
    $this->db->from('items AS items');
    $this->db->join('suppliers AS suppliers', 'suppliers.person_id = items.supplier_id', 'left');
    // Se utiliza LEFT JOIN para incluir ítems sin registros en inventory
    $this->db->join('inventory AS inventory', 'inventory.trans_items = items.item_id', 'left');
    ...
    if (!empty($search)) {
        if ($attributes_enabled && $filters['search_custom']) {
            ...
            // Búsqueda general en items: dividir en palabras y buscar en nombre, item_number, etc.
            $this->db->group_start();
                $search_words = explode(' ', trim($search));
                foreach ($search_words as $word) {
                    $this->db->like('items.name', $word);
                }
    ...
    // --- Integrar JOIN y SELECT para atributos (si están habilitados) ---
    if ($attributes_enabled) {
        $format = $this->db->escape(dateformat_mysql());
        $this->db->simple_query('SET SESSION group_concat_max_len=49152');
        $this->db->select(
            'GROUP_CONCAT(DISTINCT CONCAT_WS(\'_\', attr_links.definition_id, attr_values.attribute_value) ORDER BY attr_links.definition_id SEPARATOR \'|\') AS attribute_values',
            false
        );
        $this->db->select(
            "GROUP_CONCAT(DISTINCT CONCAT_WS('_', attr_links.definition_id, DATE_FORMAT(attr_values.attribute_date, $format)) SEPARATOR '|') AS attribute_dtvalues",
            false
        );
        $this->db->select(
            'GROUP_CONCAT(DISTINCT CONCAT_WS(\'_\', attr_links.definition_id, attr_values.attribute_decimal) SEPARATOR \'|\') AS attribute_dvalues',
            false
        );
        $this->db->join(
            'attribute_links AS attr_links',
            'attr_links.item_id = items.item_id AND attr_links.receiving_id IS NULL AND attr_links.sale_id IS NULL AND attr_links.definition_id IN (' . implode(',', $filters['definition_ids']) . ')',
            'left'
        );
        $this->db->join(
            'attribute_values AS attr_values',
            'attr_values.attribute_id = attr_links.attribute_id',
            'left'
        );
    }
    
    // --- Filtro específico para "talla" (usando join exclusivo) ---
if (!empty($talla_value)) {
    $this->db->join('attribute_links AS talla_al', 'talla_al.item_id = items.item_id', 'left');
    $this->db->join('attribute_values AS talla_av', 'talla_av.attribute_id = talla_al.attribute_id', 'left');
    // Solo considerar definiciones de tallas (por ejemplo, 2, 3, 4, 6)
    $this->db->where_in('talla_al.definition_id', array(2, 3, 4, 6));
    $this->db->group_start();
        $this->db->like('talla_av.attribute_value', $talla_value);
        $this->db->or_like('items.name', $talla_value);
    $this->db->group_end();
}
    ...
    if ($filters['empty_upc'] != FALSE) {
        $this->db->where('items.item_number', NULL);
    }
    if ($filters['low_inventory'] != FALSE) {
        $this->db->where('quantity <=', 'reorder_level');
    }
    if ($filters['is_serialized'] != FALSE) {
        $this->db->where('items.is_serialized', 1);
    }
    ...
    // --- Caso de get_found_rows ---
    if ($count_only === TRUE) {
        return $this->db->get()->row()->count;
    }
    
    // Evitar duplicados por múltiples registros en inventory
	...
	*/
	...
	public function change_unit_price($item_id, $items_received, $new_price, $old_price)
	{
		...
		//$data = array('cost_price' => $average_price);
        $data = array('cost_price' => $new_price, 'unit_price'=>$old_price);
		return $this->save2($data, $item_id);
		 
	}

	 public function save2(&$item_data, $item_id = FALSE)
	{
		if(!$item_id || !$this->exists($item_id, TRUE))
		{
			if($this->db->insert('items', $item_data))
