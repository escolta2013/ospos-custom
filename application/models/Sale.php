...
class Sale extends CI_Model
{
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
	{
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_payments_temp') .
			...
			(
				SELECT payments.sale_id,
					...
					GROUP_CONCAT(IF(payments.M1>0, CONCAT(payments.payment_type, ": ", payments.M1," / ", payments.T1, "=" ,payments.payment_amount- payments.cash_refund), CONCAT(payments.payment_type, " ", (payments.payment_amount - payments.cash_refund))) SEPARATOR " | ") AS payment_type,  MAX(payments.M1) AS M1, MAX(payments.T1) as T1
		...
		else
		{
			$this->db->select('
					...
					" . '
					MAX(payments.payment_type) AS payment_type,
					MAX(payments.M1) AS M1,
                    MAX(payments.T1) AS T1
			');
		}
	...
	{
		// get payment summary
		$this->db->select('payment_type, T1, COUNT(payment_amount) AS count, SUM(payment_amount - cash_refund) AS payment_amount');
		$this->db->from('sales AS sales');
		$this->db->join('sales_payments', 'sales_payments.sale_id = sales.sale_id');
		$this->db->join('people AS customer_p', 'sales.customer_id = customer_p.person_id', 'LEFT');
		$this->db->join('customers AS customer', 'sales.customer_id = customer.person_id', 'LEFT');

		// @Nukerj Filtro por ubicación 
		if($filters['location_id'] !='all')
		{
		 
		$this->db->where('sales.location', $filters['location_id']);
		}
	...
	public function get_sale_tasa($sale_idv)
	{
		
		$this->db->from('sales');
        $this->db->where('sale_id', $sale_idv);
        ...
        if(empty($result) == TRUE)
        {
            ...
            foreach($this->db->list_fields('tasa') as $field)
            {
                $result->$field = '';
            }
			...
			{				
				...
				if($payment_id == -1 && $payment_amount != 0)
				{
					...
					$sales_payments_data = array(
						...
						'M1'=>$payment['payment_amount']*str_replace(",", ".", $this->config->item('tasa')),
						'T1'=>str_replace(",", ".", $this->config->item('tasa')),
							...
							$work_order_number, $quote_number, $sale_type, $payments, $dinner_table, &$sales_taxes)
	{
		...
		foreach($sales_taxes[1] as $line => $tax_item)
		{   
			if ($tax_item['percent']>0) {
			  $impuesto+=$tax_item['item_tax_amount'];
			}
       ...
       foreach($items as $item2)
		{
         $subtotal=($item2['quantity']*$item2['price']);
         $total+=$impuesto+$subtotal-($subtotal*$item2['discount']/100);

			$sales_data = array(
				'sale_time'			=> date('Y-m-d H:i:s'),
				'customer_id'		=> $this->Customer->exists($customer_id) ? $customer_id : NULL,
				'employee_id'		=> $employee_id,
				'monto'			    =>$total, // CAMBIO 4
				'comment'			=> $comment,
				...
				'sale_status'		=> $sale_status,
				'sale_type'			=> $sale_type,
				'tasa'			    => str_replace(",", ".", $this->config->item('tasa')),
			);
		 }
	...
	public function get_payment_options($giftcard = TRUE, $reward_points = FALSE)
	{
		...
		if($giftcard == TRUE)
		{
			if ( $this->sale_lib->get_method()=="Mixto"|| $this->sale_lib->get_method()=="Dolares") {
	...
	public function save_payment_tocredit($sale_id, $amount, $id = null, $payment_typeadd)
	{
		if(strpos($id, 'F20') === FALSE)
			$id = 'F'.date('YmdHis');

		  
         $sata_deudas= $this->Billtocollect->get_deudas($sale_id);
         $data_sale= $this->Billtocollect->get_data_sale($sale_id);

          if ($payment_typeadd=='CashBs' || $payment_typeadd=='Debit' || $payment_typeadd=='PGM') {
        	
        	$tasa = $this->config->item('tasa');
            $tasa = str_replace(',', '.', $tasa);
        	$add_amount=$amount/$tasa;
		...
		$payment_tocredit = array(
			'sale_id'			=> $sale_id,
			'payment_time'       => date('Y-m-d H:i:s'), // nuker 12-11-22
			'payment_tocredit'	=> $id,
			'payment_amount'	=> $add_amount,
            'T1'	=> $tasa,
			'customer_id'=>$data_sale->customer_id,
			'employee_id'=>$data_sale->employee_id,
			'payment_type'=>$payment_typeadd,
			'payment_amount_credit'	=> $sata_deudas->payment_amount,
			'sale_status' =>$data_sale->sale_status,
			'status_credito' =>0
		...
		if($amount >0){
           ...
           $this->Billtocollect->update_estatus_abonado($sale_id, $status);
           //$this->Billtocollect->update_estatus_abonado_p($sale_id, $status); 
		}

		
         
         // nuker 13-11-22 Desactiva el estatus credito cuando la deuda esta pagada  
		 $abonos= $this->Billtocollect->get_abonos($sale_id);
		 $deudas= $this->Billtocollect->get_deudas($sale_id); 

         if ($abonos->payment_amount>=$deudas->payment_amount) {
           $status= array(
          'status_credito' =>1 , );	
          $this->Billtocollect->update_estatus_credit($sale_id, $status); 
          $this->Billtocollect->update_estatus_credit_p($sale_id, $status); 
         }
	...
	public function get_payments_tocredit_recibed($sale_id)
	{
		$this->db->select('sales_payments_tocredit.sale_id as sale_id, sales_payments_tocredit.payment_tocredit as payment_tocredit, sales_payments_tocredit.payment_amount as payment_amount, sales_payments_tocredit.payment_time as fecha_pago');
		$this->db->from('sales_payments_tocredit');
	...
	public function get_payments_prima($sale_id)
	{
		$this->db->select('payment_amount, payment_type');
		$this->db->from('sales_payments');  
		$this->db->where('payment_type<>', $this->lang->line('sales_tocredit'));
		$this->db->where('sale_id', $sale_id); 
		 
		$payments_prima = $this->db->get()->result_array(); 
		return $payments_prima;
	}

	 public function get_info_list_prod($sale_id)
	{   
	...
	public function get_sales_items_crons(){
		$this->db->select('distinct(sale_id) AS sale_id, item_location');   
		$this->db->from('sales_items'); 
		$this->db->order_by('sale_id');  
		return  $this->db->get()->result_array();
	}

	public function update_location($sale_id, $add_value)
	{   
		$this->db->where('sale_id', $sale_id);
		$this->db->update('sales', $add_value); 
	} 
