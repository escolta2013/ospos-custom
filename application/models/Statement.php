...
class Statement extends CI_Model
{
	...
	{
		return $this->search($search, $filters, 0, 0, 'payments_tocredit.payment_tocredit', 'desc', TRUE);
	}

	/**
	 * Get the sales data for the takings (sales/manage) view
	 */
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'payments_tocredit.payment_tocredit', $order = 'desc', $count_only = FALSE)
	{
		// Pick up only non-suspended records and

		// nuker 12-11-22 filtrar solo si hay abonos
		$where = 'payments_tocredit.payment_amount > 0 AND '; 
		//$where3='payments_tocredit.status_credito=0'; 
		$where_delete = 'payments_tocredit.sale_status = 0'; 


		if(empty($this->config->item('date_or_time_format')))
		{
			$where .= 'DATE(payments_tocredit.payment_time) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']);
		}
		else
		{
			$where .= 'payments_tocredit.payment_time BETWEEN ' . $this->db->escape(rawurldecode($filters['start_date'])) . ' AND ' . $this->db->escape(rawurldecode($filters['end_date']));
		} 

		if(empty($this->config->item('date_or_time_format')))
		{
			$fecha .= 'DATE(payments_tocredit.payment_time) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']);
		}
		else
		{
			$fecha .= 'payments_tocredit.payment_time BETWEEN ' . $this->db->escape(rawurldecode($filters['start_date'])) . ' AND ' . $this->db->escape(rawurldecode($filters['end_date']));
		} 


        // nuker 12-11-22 Permite calcular los abonos y monto adeudado de forma correcta 	
		...
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(DISTINCT payments_tocredit.payment_tocredit) AS count');
		}else{
			$this->db->select('
				payments_tocredit.sale_id AS sale_id,
				MAX(DATE_FORMAT(payments_tocredit.payment_time, "%d-%m-%Y" )) AS sale_date,
				MAX(payments_tocredit.customer_id) as customer_id,  
				MAX(payments_tocredit.employee_id) as employee_id,  
				MAX(payments_tocredit.status_credito) as status_credito, 
				MAX(payments_tocredit.payment_tocredit) as payment_tocredit,
				MAX(payments_tocredit.payment_type) as payment_type,
				MAX(payments_tocredit.T1) as T1,
				MAX(payments_tocredit.payment_time) as payment_time, 
				SUM(payments_tocredit.payment_amount) as payment_amount,
				SUM(payments_tocredit.payment_amount_credit) as payment_amounts,			 
		...
		$this->db->from('sales_payments_tocredit AS  payments_tocredit'); 

		$this->db->join('people AS customer', ' payments_tocredit.customer_id = customer.person_id', 'LEFT'); 

		$this->db->join('people AS employee', ' payments_tocredit.employee_id = employee.person_id', 'LEFT');  
		...
		{
			$this->db->where('payments_tocredit.employee_id', $filters['employee_id']);
		} 

		if($filters['customer_id'] != 'all')
		{
			$this->db->where('payments_tocredit.customer_id', $filters['customer_id']);
		} 
		...
		$this->db->group_by('payments_tocredit.payment_tocredit');

		// order by sale time by default
		$this->db->order_by('payments_tocredit.customer_id, payments_tocredit.payment_time, payments_tocredit.payment_type','desc');
	...
	public function get_payments_tc_summary($search, $filters)
	{
		...
		$this->db->join('sales AS sales', 'payments_tocredit.sale_id = sales.sale_id', 'LEFT');
		$this->db->join('people AS employee_p', 'sales.employee_id = employee_p.person_id', 'LEFT'); 
		$this->db->join('people AS customer_p', 'payments_tocredit.customer_id = customer_p.person_id', 'LEFT');
		$this->db->join('customers AS customer', 'payments_tocredit.customer_id = customer.person_id', 'LEFT');		
		$this->db->join('employees AS employee', 'sales.employee_id = employee.person_id', 'LEFT');



		//$this->db->where('payments_tocredit.payment_type', $this->lang->line('sales_due'));
		...
		if(empty($this->config->item('date_or_time_format')))
		{
			$this->db->where('DATE(payments_tocredit.payment_time) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']));
		}
		else
		{
			$this->db->where('payments_tocredit.payment_time BETWEEN ' . $this->db->escape(rawurldecode($filters['start_date'])) . ' AND ' . $this->db->escape(rawurldecode($filters['end_date'])));
		}
	...
	public function get_payments_summary($search, $filters)
	{
		...
		if(!empty($search))
		{
			if($filters['is_valid_receipt'] != FALSE)
			{
				...
				$this->db->where('payments_tocredit.sale_id', $pieces[1]);
			}
			else
			{
				$this->db->group_start();
...
// 	/cambio para ventas del dia y mes.. completas y suspendidas
	...
	public function update_status_sales($sale_id, $status_payment)
	{  

		$this->db->where('sale_id', $sale_id);
		$this->db->update('sales', $status_payment);

	}

	public function update_status_rev($sale_id)
	{   
		$sale_data= array(
			'status' =>0
		); 
		...
		$this->db->where('sale_id', $sale_id); 
		$this->db->update('sales_payments', $sale_data);

	}

	public function delete_pago($payment_tocredit)
	{  

		$this->db->where('payment_tocredit', $payment_tocredit);
		$this->db->delete('sales_payments_tocredit', array('payment_tocredit' => $payment_tocredit));
