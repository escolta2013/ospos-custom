...
class Customer extends Person
{
	...
	*/
	public function get_search_suggestions($search, $unique = TRUE, $limit = 25)
	{
		...
		$this->db->group_start();
			...
			$this->db->or_like('cedula', $search); // Nukerj 14-01-2023
	...
	*/
	...
	public function get_stats_accountan($customer_id,$filter = NULL)
	{
		$this->db->select('
						SUM(sales_payments.payment_amount - sales_payments.cash_refund) AS due_amount, 
						
						(SELECT SUM(ospos_sales_payments.payment_amount) from ospos_sales JOIN ospos_sales_payments WHERE ospos_sales_payments.sale_id = ospos_sales.sale_id and ospos_sales.customer_id='.$customer_id.' and ospos_sales.sale_status =0 and ospos_sales_payments.payment_type="'.$this->lang->line('sales_due').'") AS total_credits,

						IFNULL( ( SELECT SUM(b.payment_amount) AS abonos FROM ' . $this->db->dbprefix('sales') . ' as sales LEFT JOIN ' . $this->db->dbprefix('sales_payments_tocredit') . ' AS b ON b.sale_id = sales.sale_id WHERE sales.sale_status = 0 AND sales.customer_id = ' . $this->db->escape($customer_id) . '),0) AS due_tocredit
						');
		$this->db->from('sales');
		$this->db->join('sales_payments AS sales_payments', 'sales.sale_id = sales_payments.sale_id');
