...
class Attribute extends CI_Model
{
	...
	{
	$where_clause = '(definition_flags & ' . (int)$definition_flags . ') = ' . (int)$definition_flags;
	...
	public function get_link_values($item_id, $sale_receiving_fk, $id, $definition_flags)
{
    ...
    {
        // Revisa estas líneas: si se pretende usar estas condiciones como cláusula personalizada, quizás
        // necesites combinarlas o definirlas explícitamente. Por ejemplo:
        // $this->db->where("sale_id IS NULL AND receiving_id IS NULL", NULL, FALSE);
        $this->db->where('sale_id');
        $this->db->where('receiving_id');
    }
