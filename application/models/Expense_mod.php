...
class Expense extends CI_Model
{
    ...
    public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'expense_id', $order = 'asc', $count_only = FALSE)
    {
        ...
        if ($count_only == TRUE) {
            ...
            $this->db->select('
                ...
                COALESCE(suppliers.company_name, expenses.supplier_name) AS supplier_name, -- Prioriza supplier_name si no hay supplier_id
        ...
        $this->db->join('suppliers AS suppliers', 'suppliers.person_id = expenses.supplier_id', 'LEFT'); // Join opcional
        ...
        $this->db->or_like('COALESCE(suppliers.company_name, expenses.supplier_name)', $search); // Buscar en supplier_name también
        $this->db->group_end();
    ...
    public function get_info($expense_id)
    {
        $this->db->select('
            ...
            expenses.supplier_name AS supplier_name_manual, -- Nuevo campo para mostrar el nombre manual
    ...
    {
        // Si no hay supplier_id, usa supplier_name
        if (!isset($expense_data['supplier_id']) || empty($expense_data['supplier_id'])) {
            $expense_data['supplier_name'] = $expense_data['supplier_name'] ?? null;
        }
