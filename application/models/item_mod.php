...
class Item extends CI_Model
{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
        ...
        $this->db->where('location_id', $filters['location_id']);
    	}
	...
	*/
	public function get_all($stock_location_id = -1, $rows = 0, $limit_from = 0, $employee_location_id = -1)
	{
    $this->db->from('items');

    // Filtro original de stock_location (inventario)
    if($stock_location_id > -1)
    {
        $this->db->join('item_quantities', 'item_quantities.item_id = items.item_id');
        $this->db->where('item_quantities.location_id', $stock_location_id); // Mejor explícito con la tabla
    }

    // === FILTRO NUEVO POR TU location_id (tienda) ===
    if ($employee_location_id > -1) {
        $this->db->where('items.location_id', $employee_location_id); // Usa la columna de tu tienda
    }
	...
	public function get_all_by_employee_location()
	{
    $employee_location = $this->Employee->get_logged_in_employee_current_location_id();
    $is_admin = $this->Employee->has_grant('admin');
    ...
    if (!$is_admin) {
        $this->db->where('location_id', $employee_location);
    }
    ...
    {
        $this->db->select('items.*, location_id');
        $this->db->from('items');
        $this->db->join('people', 'people.person_id = items.person_id');
    ...
    public function get_items_by_location($location_id)
    {
	...
	*/
	...
	public function get_item_location($item_id)
	{
    ...
    return $query->row()->location_id ?? 0; // Si no existe, retorna 0
	}
