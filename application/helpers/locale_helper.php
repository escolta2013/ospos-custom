...
function get_payment_options()
{
    ...
    // Intentar obtener el m«±todo de pago de forma segura
    $method = '';
    if (isset($CI->sale_lib) && is_object($CI->sale_lib) && method_exists($CI->sale_lib, 'get_method')) {
        $method = $CI->sale_lib->get_method();
    }
    // Si no se obtiene m«±todo, asignar un valor predeterminado para que la lista no quede vac«¿a.
    if (empty($method)) {
        // Puedes cambiar este valor seg«ân tus necesidades (por ejemplo, "Dolares" o "Mixto")
        $method = "Dolares";
    }

    if ($config->item('payment_options_order') == 'Morsas')
    {
        if ($method === "Mixto" || $method === "Dolares") {
            $payments['Cash']   = '$ Efectivo'; // Efectivo en D«Ñlares
            $payments['zelle']  = 'Zelle';
            $payments['venmo']  = 'Venmo';
            $payments['paypal'] = 'PayPal';
            $payments['interDo'] ='Intercambio $';
            //$payments[$lang->line('sales_credit')] = $lang->line('sales_credit');
        }

        if ($method === "Mixto" || $method === "Bolivares") {
            $payments['CashBs'] = 'Bs. Efectivo'; // Efectivo en Bol«¿vares
            $payments['Debit']  = 'Punto de Venta';
            $payments['PGM']    = 'Pago Movil';
            $payments['inter']  = 'Intercambio Bs';
            $payments[$lang->line('sales_due')] = $lang->line('sales_due');
        }
    }

    if ($method === "Mixto" || $method === "Dolares") {
        if ($config->item('payment_options_order') == 'debitcashcredit')
        {
        ...
        else { // Default: si es 'cashdebitcredit' u otro valor
            //[$lang->line('sales_debit')]  = $lang->line('sales_debit');
           // $payments[$lang->line('sales_credit')] = $lang->line('sales_credit');
        }
        ...
        // Si el pa«¿s incluye India, se a«Ðade UPI
        if (stripos($config->item('country_codes'), 'IN') !== false)
        {
            $payments[$lang->line('sales_upi')] = $lang->line('sales_upi');
        }
    }

    // Si a«ân no se generaron opciones, asignar una opci«Ñn por defecto
    if (empty($payments)) {
