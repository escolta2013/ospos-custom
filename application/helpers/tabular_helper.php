...
function transform_headers($array, $readonly = FALSE, $editable = TRUE)
{
	...
	foreach($array as $element)
	{
		...
		$result[] = array('field' => key($element),
			...
			'escape' => !preg_match("/(dell|fact|edit|phone_number|email|messages|item_pic)/", key($element)) && !(isset($element['escape']) && !$element['escape']),
...
function get_sales_manage_payments_summary($payments)
{
	...
	foreach($payments as $key=>$payment)
	{
		...
		$total = bcadd($total, $amount); 
			...
			if($payment['payment_type']=="CashBs" || $payment['payment_type']=="PGM" || $payment['payment_type']=="Debit"){

				$table .= '<div class="summary_row">' . $payment['payment_type'] . ': ' . to_currency($amount*$payment['T1'])."=".to_currency($amount) . '</div>';
...
function get_customer_manage_table_headers()
{
	...
	$headers = array(
		...
		array('last_name' => $CI->lang->line('common_last_name')),
		array('first_name' => $CI->lang->line('common_first_name')),
		array('cedula' => $CI->lang->line('common_cedula')),
		array('email' => $CI->lang->line('common_email')),
		array('phone_number' => $CI->lang->line('common_phone_number')),
		array('total_due' => $CI->lang->line('sales_total_credits'), 'sortable' => FALSE),
		array('total_credit' => $CI->lang->line('sales_amount_credits'), 'sortable' => FALSE),
		array('rest_credits' => $CI->lang->line('sales_amount_rest_credits'), 'sortable' => FALSE),
...
*/
function get_customer_data_row($person, $stats, $stats_due)
{
	$CI =& get_instance();
	...
	return array (
		...
		'email' => empty($person->email) ? '' : mailto($person->email, $person->email),
		'phone_number' => $person->phone_number,
	    'total_due' => to_currency($stats_due->total_credits),
		'total_credit' => to_currency($stats_due->due_tocredit),
		'rest_credits' => to_currency( $stats_due->total_credits - $stats_due->due_tocredit ),
...
function get_expenses_manage_table_headers()
{
    ...
    $headers = array(
        ...
        array('due_date' => $CI->lang->line('expenses_due_date')),          // Nueva cabecera
        array('payment_status' => $CI->lang->line('expenses_payment_status')), // Nueva cabecera
        array('amount_due' => $CI->lang->line('expenses_amount_due')),      // Nueva cabecera
...
function get_expenses_data_row($expense)
{
    ...
    return array (
        ...
        'due_date' => to_date(strtotime($expense->due_date)),               // Nuevo campo
        'payment_status' => $CI->lang->line('expenses_' . $expense->payment_status), // Nuevo campo (traducido)
        'amount_due' => to_currency($expense->amount_due),                  // Nuevo campo
...
/* nuker 13-11-22 */
function get_billtocollect_manage_table_headers()
{
	$CI =& get_instance();

	$headers = array(
		array('sale_id' => $CI->lang->line('sales_invoice')),
		array('sale_date' => $CI->lang->line('sales_date')),
		array('employee_name' => $CI->lang->line('employees_employee')),
		array('customer_name' => $CI->lang->line('customers_customer')),
		array('payment_amounts' => $CI->lang->line('sales_total_credito')),
		array('abonos' => $CI->lang->line('statements_paymentamount')),
		array('deudado' => $CI->lang->line('config_saldo')),
		array('fact' => ''), 
...
/* nuker 13-11-22 */ 
function get_billtocollect_data_row($billtocollect)
{
	$CI =& get_instance();

	$controller_name = $CI->uri->segment(1);
     
    $model = get_instance()->Billtocollect;
    $model->get_abonos($billtocollect->sale_id)->payment_amount;
    
    if (is_null($billtocollect->sale_id)) {
     $abonos='';
    } else {
      $abonos=$model->get_abonos($billtocollect->sale_id)->payment_amount;
    } 
    
 
	 $saldo_a= $abonos>0 ? $billtocollect->payment_amounts-$abonos :$billtocollect->payment_amounts;
     $saldo_b=$$abonos>0 ? to_currency_no_money($billtocollect->payment_amounts) : $billtocollect->payment_amounts;
     $tasa = str_replace(',', '.',$CI->config->item('tasa'));
	$row = array (
		'sale_id' => $billtocollect->sale_id, 
		'sale_date' => $billtocollect->sale_date,
		'employee_name' => $billtocollect->employee_name, 
		'customer_name' => $billtocollect->customer_name, 
		'payment_amounts' => $billtocollect->payment_amounts>0 ? to_currency($billtocollect->payment_amounts).' =Bs. '.to_currency_no_money($billtocollect->payment_amounts*$tasa) :'' ,
		'abonos' => $abonos>0 ? to_currency($abonos) :'', 
		'deudado' => $billtocollect->payment_amounts>0 ? $saldo_a.' =Bs.'.to_currency_no_money($saldo_a*$tasa)  : $saldo_b, 
		 		 
	);
  
      $fecha_venta=date_create($billtocollect->sale_date);
	  if ($billtocollect->payment_amounts) {
 	   
			  	$row['edit'] = anchor(
			  		'sales/edit/'.$billtocollect->sale_id.'/2',
			  		'<span class="btn btn-info rounded-pill btn-sm"><img src="images/billtocollects/pay.svg" width="25">'.$CI->lang->line("statements_pay").'</span>',
			  		array(
			  			'class' => 'btn modal-dlg',
			  			'data-btn-submit' => $CI->lang->line("statements_pay"),
			  			'title' => 'Abono Creditos', 
		    )
			  	);
	 
	     

	      $row['fact'] = anchor("sales/receipt/$billtocollect->sale_id/", '<span class="btn btn-info rounded-pill btn-sm"><img src="images/billtocollects/receipt.svg" width="25"> '.$CI->lang->line("statements_impinvoice").'</span>',
			array( 'target'=>'_blank'  ));
...
function get_billtocollect_data_last_row($billtocollect, $payments, $payments_tc)
{
	$CI =& get_instance();
	...
	foreach($payments_tc as $key2=>$payment_c)
	{
		$amountd = $payment_c['payment_amount'];
		$totald = bcadd($totald, $amountd);
	...
	return array(
		...
		'employee_name' => '', 
		'customer_name' => '', 
		'payment_amounts' => to_currency($amount).'= Bs. '.to_currency_no_money($amount*$tasa), 
		'abonos' => to_currency($totald).'= Bs. '.to_currency_no_money($totald*$tasa), 
		'deudado' =>to_currency($total-$totald).'= Bs. '.to_currency_no_money(($total-$totald)*$tasa), 
		'fact' => '', 
		'pay' => '', 
	);
}
...
/* nuker 13-11-22 */
function get_billtocollect_manage_payments_summary($payments, $payments_tc, $sales)
{
	$CI =& get_instance();
    ...
    $table .= '<div class="summary_row">'.$CI->lang->line('statements_paymentamount').' ' . to_currency($totald) . '</div>';
	$table .= '<div class="summary_row">' . $CI->lang->line('sales_total') . ': ' . to_currency($total-$totald) . '</div>';
	$table .= '</div>';
...
nuker 12-11-22
*/
function get_statemens_manage_table_headers()
{
	$CI =& get_instance();

	$headers = array(
		array('sale_id' => $CI->lang->line('sales_invoice')),
		array('sale_date' => $CI->lang->line('statements_payment_time')),
		array('employee_name' => $CI->lang->line('employees_employee')),
		array('customer_name' => $CI->lang->line('customers_customer')),
		array('payment_tocredit' => $CI->lang->line('statements_recibo')), 
		array('payment_amount' => $CI->lang->line('statements_paymentamount')), 
...
/* nuker 12-11-22 */
function get_statements_data_row($statement)
{
	$CI =& get_instance();

	$controller_name = $CI->uri->segment(1);

	$model = get_instance()->Statement;
    $model->get_abonos($statement->sale_id)->payment_amount;
    
    if (is_null($statement->sale_id)) {
     $abonos='';
    } else {
      $abonos=$model->get_abonos($statement->sale_id)->payment_amount;
    } 
     

     if ($statement->payment_type=="CashBs" || $statement->payment_type=="Debit" || $statement->payment_type=="PGM") {        	
          $tipopago=1;
        } else {
          $tipopago=0; 
        }
	...
	$row = array (
		...
		'sale_date' => $statement->sale_date,
		'employee_name' => $statement->employee_name, 
		'customer_name' => $statement->customer_name, 
		'payment_tocredit' => $statement->payment_tocredit , 
		'payment_amount' => $tipopago==1 ? to_currency_no_money($statement->payment_amount*$statement->T1).'='.to_currency($statement->payment_amount) : to_currency($statement->payment_amount),
		'payment_type' => $statement->payment_type, 	 
	);

	 
	  if ($statement->payment_tocredit) {
 	     
        if($CI->Employee->has_grant('statements_receipt', $CI->session->userdata('person_id')))
	       { 
	        $row['edit'] = anchor("/sales/edit/$statement->sale_id/1/$statement->payment_tocredit/$statement->sale_date/", '<span class="btn btn-info rounded-pill btn-sm"><img src="images/billtocollects/receipt.svg" width="25">  '.$CI->lang->line("statements_impinvoice").'</span>', array( 'target'=>"_blank" ) ); 
	      }
	   
         
         if($CI->Employee->has_grant('statements_dell', $CI->session->userdata('person_id')))
	       {

		    $row['dell'] = anchor("/statements/dell_pago/$statement->payment_tocredit/$statement->sale_id/", '<span class="btn btn-info rounded-pill btn-sm"><img src="images/billtocollects/borrador.svg" width="25">'.$CI->lang->line("statements_dell").'</span>'  );
		   } 
...
/*nuker 12-11-22*/
function get_statements_manage_payments_summary($payments, $payments_tc , $sales)
{
	$CI =& get_instance();
