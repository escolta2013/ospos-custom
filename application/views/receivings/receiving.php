	...
	<table class="sales_table_100" id="register">
		<thead>
			<tr>
				...
				<th style="width:3%;"><?php echo $this->lang->line('porcentaje_venta'); ?></th>
				<th style="width:14%;"><?php echo $this->lang->line('receivings_price'); ?></th>
				<th style="width:14%;"><?php echo $this->lang->line('receivings_cost'); ?></th>
			...
			?>
					<?php echo form_open($controller_name."/edit_item/$line", array('class'=>'form-horizontal', 'id'=>'cart_'.$line)); ?>
						<tr>
							...
							<td><?php echo form_input(array('onKeyUp'=>"calcular(this);", 'onclick'=>"value=''",'name'=>'porcentaje', 'id'=>'porcentaje_'.$line , 'class'=>'form-control input-sm', 'value'=>to_currency_no_money($item['porcentaje'])));?></td>

							<td><?php echo form_input(array('onKeyUp'=>"calcular(this);", 'onclick'=>"value=''",'name'=>'cost', 'id'=>'cost_'.$line  , 'class'=>'form-control input-sm', 'value'=>to_currency_no_money($item['cost'])));?></td>
							...
							{
							?>
								<td><?php echo form_input(array('name'=>'price', 'onKeyUp'=>"calcular(this);",  'id'=>'price_'.$line, 'class'=>'form-control input-sm', 'value'=>to_currency_no_money($item['price']),'onClick'=>'this.select();'));?></td>
							<?php
							}
							else
							{
...
function calcular(input) {
  ...
  for (var i = 0; i < counter; i++) {
    ...
    var costo = parseFloat(quitarSeparadores(document.getElementById("price_" + cant).value));


    if (id == "cost_" + cant) {

      base_imponible = document.getElementById("cost_" + cant).value / document.getElementById("price_" + cant).value; 
     
      var newValue = ((base_imponible - 1) * 100).toFixed(2);

       if (isNaN(newValue)) {
        newValue = 0; // Mantener el valor actual del input
      }

    

      document.getElementById("porcentaje_" + cant).value = newValue;

    } else if (id == "porcentaje_" + cant) {
      var newValue = ((input.value * costo / 100) + costo).toFixed(2);
      
      if (isNaN(newValue)) {
        newValue = 0; // Mantener el valor actual del input
      }

      document.getElementById("cost_" + cant).value = newValue;
    } else if (id == "price_" + cant) {
      var newValue = ((document.getElementById("porcentaje_" + cant).value * costo / 100) + costo).toFixed(2);
      if (isNaN(newValue)) {
        newValue = 0; // Mantener el valor actual del input
      }
      document.getElementById("cost_" + cant).value = newValue;
...
function quitarSeparadores(numero) {
...
$(document).ready(function()
{
	...
	$('[name="porcentaje"], [name="cost"], [name="price"],[name="quantity"],[name="receiving_quantity"],[name="discount"],[name="description"],[name="serialnumber"]').change(function() {
		$(this).parents("tr").prevAll("form:first").submit()
	});
