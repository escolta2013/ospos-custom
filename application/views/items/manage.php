...
<?php
// =============================================
// INICIO MODIFICACIÓN: Ocultar columna de costo
// =============================================
$employee_info = $this->Employee->get_logged_in_employee_info();
$is_admin = $employee_info->is_admin ?? false;
$headers = json_decode($table_headers, true);

if (!$is_admin) {
    foreach ($headers as &$header) {
        if ($header['field'] === 'cost_price') {
            $header['visible'] = false;     // Oculta la columna
            $header['switchable'] = false;  // Elimina del selector de columnas
            $header['checkboxable'] = false;// Evita checkboxes
        }
    }
}
...
$(document).ready(function() {
    ...
    // =============================================
    // INICIO MODIFICACIÓN: Deshabilitar memorización
    // =============================================
     localStorage.removeItem('bs.table.columns'); // Borra preferencias anteriores
   ...
   table_support.init({
    ...
    pageSize: <?php echo $this->config->item('lines_per_page') ?? 25; ?>,
    pageList: [10, 25, 50, 100, 200], // Coma agregada
...
<!-- Resto del código HTML (no requiere cambios) -->
...
<script src="<?php echo base_url('js/block_update.js'); ?>"></script> 
