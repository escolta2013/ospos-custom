...
class Tax_lib
{
		...
		if($customer_id == -1 || $customer_info->taxable)
		{
			foreach($cart as $line => $item)
			{
				...
				if(!($this->CI->config->item('use_destination_based_tax')))
				{
					...
					foreach($this->CI->sale_lib->get_payments() as $payments)
					{
						if($payments['payment_type']=="CashBs" || $payments['payment_type']=="PGM" || $payments['payment_type']=="Debit"){
							$p1=1;
							$p2=1;
							$p3=1;
					...
					{
						if ($this->CI->sale_lib->get_igtf()==1 && $tax['name']=="IGTF") {
							$tax['percent']=$tax['percent'];
							$taxigtf=$tax['percent']/100;
						} else if ($this->CI->sale_lib->get_igtf()==0 && $tax['name']=="IGTF") {
							$tax['percent']=0;
							$taxigtf=0; 
						}

						if ($this->CI->sale_lib->get_iva()==1 && $tax['name']=="IVA") {
							$tax['percent']=$tax['percent'];
							$taxiva=$tax['percent'];
						} else if ($this->CI->sale_lib->get_iva()==0 && $tax['name']=="IVA") {
							$tax['percent']=0; 
							$taxiva=$tax['percent'];
						}
						
						if ($tax['name']=="IGTF" && $p1==1 && $p2==2 && $p3==3) {
							$tax['percent']=0;
						}else{
							$tax['percent']=$tax['percent'];
						}

						if ( $this->CI->sale_lib->get_method()=="Mixto"|| $this->CI->sale_lib->get_method()=="Dolares" && $tax['name']=="IGTF") { 
							$tax['percent']=$tax['percent'];
						} else if ( $this->CI->sale_lib->get_method()!="Dolares" && $tax['name']=="IGTF") {
							$tax['percent']=0;
							$this->set_val_igtf(0); 
						}
						...
						{ 
                            ...
                            if ($this->CI->sale_lib->get_igtf()==1 && $this->CI->sale_lib->get_iva()==1) {
                            	$tax_amountadd =round(doubleval($tax_amount*$taxigtf),2);
                            	$this->set_val_igtf($tax_amountadd); 
                            } 
							...
							if ($tax['name']=="IGTF"){ 
								...
								if ($this->CI->sale_lib->get_igtf()==1 && $this->CI->sale_lib->get_iva()==1) {
									$tax_amount = $this->get_tax_for_amount($tax_basis, $tax['percent'], Rounding_mode::HALF_UP, $tax_decimals)+$this->get_val_igtf();
								}else  if ($this->CI->sale_lib->get_igtf()==1 && $this->CI->sale_lib->get_iva()==0) {
									$tax_amount = $this->get_tax_for_amount($tax_basis, $tax['percent'], Rounding_mode::HALF_UP, $tax_decimals);
								} 
							}
	...
	public function set_val_igtf($tax_amountadd)
	{
		$this->CI->session->set_userdata('tax_amountadd', $tax_amountadd);
	}

	public function get_val_igtf()
	{
		return $this->CI->session->userdata('tax_amountadd');
	}
