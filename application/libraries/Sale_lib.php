...
class Sale_lib
{
	...
	{
		$this->CI->session->set_userdata('act_iva', 1);
        $this->CI->session->set_userdata('act_igtf', 1);
        $this->CI->session->set_userdata('tax_amountadd');
	}
	...
	{   
		...
		$tasa=(float)  str_replace(',', '.',$this->CI->config->item('tasa')); 
		$payments = $this->get_payments(); 

		if(isset($payments[$payment_id]))
		{ 
			if($payment_id=="CashBs" || $payment_id=="PGM" || $payment_id=="Debit"){
             //payment_method already exists, add to payment_amount
			$payments[$payment_id]['payment_amount'] = bcadd($payments[$payment_id]['payment_amount'], $payment_amount/$tasa);
            }else{
            	//payment_method already exists, add to payment_amount
		...
		{   
			if($payment_id=="CashBs" || $payment_id=="PGM" || $payment_id=="Debit"){
				if ($this->CI->router->fetch_method()=="receipt") {
					 //add to existing array
			$payment = array($payment_id => array('payment_type' => $payment_id, 'payment_amount' => $payment_amount, 
				'cash_refund' => 0, 'cash_adjustment' => $cash_adjustment, 'M1' => ($payment_amount*$tasa)));	
				} else {
					 //add to existing array
			$payment = array($payment_id => array('payment_type' => $payment_id, 'payment_amount' => number_format($payment_amount/$tasa,2), 
				'cash_refund' => 0, 'cash_adjustment' => $cash_adjustment, 'M1' => ($payment_amount*$tasa)));	
				}
	...
	{   
		...
		{  
			...
			$payments[$payment_id]['payment_amount'] = $payment_amount/$tasa;
		    }else{
		    $payments[$payment_id]['payment_type'] = $payment_id;
	...
	public function get_payments_total()
{
    ...
    foreach($this->get_payments() as $payment_id => $payments) // CORREGIDO: Ahora obtenemos payment_id
    {
        ...
        if($this->CI->lang->line('sales_cash') != $payments['payment_type'] && 
           $this->CI->lang->line('sales_cash_adjustment') != $payments['payment_type'])
        {
	...
	public function set_iva($act_iva)
	{
		$this->CI->session->set_userdata('act_iva', $act_iva);
	}

	public function get_iva()
	{
		return $this->CI->session->userdata('act_iva');
	}

	public function set_igtf($act_igtf)
	{
		$this->CI->session->set_userdata('act_igtf', $act_igtf);
	}

	public function get_igtf()
	{
		return $this->CI->session->userdata('act_igtf');
	}
	...
	public function set_method($payment_method)
	{
		$this->CI->session->set_userdata('payment_method', $payment_method);
	}

	public function get_method()
	{
		if(!$this->CI->session->userdata('payment_method'))                                                                
		{   
			$this->set_method('Mixto'); 
		}      
		return $this->CI->session->userdata('payment_method');
	}
	...
	public function clear_method()
	{
		$this->CI->session->unset_userdata('payment_method');
	}
