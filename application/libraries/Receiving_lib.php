...
class Receiving_lib
{
	...
	public function add_item($item_id, $quantity = 1, $item_location = NULL, $discount = 0, $porcentaje = NULL, $discount_type = 0, $price = NULL, $cost = NULL, $description = NULL, $serialnumber = NULL, $receiving_quantity = NULL, $receiving_id = NULL, $include_deleted = FALSE)
	{
		...
		// By @Nukerj Obtener pvp


		$cost =  $this->CI->Item->get_info($item_id)->unit_price;
		
		// Calculo de Porcentaje Articulo
		if ($price != 0) 
		{
            $base_imponible = $cost / $price;
            $porcentaje = (($base_imponible - 1) * 100);
        } 
        else 
        {
            $base_imponible = 0;
            $porcentaje = 0; // O cualquier otro valor predeterminado que tenga sentido en tu lógica
        }
		...
		$item = array($insertkey => array(
				...
				'price' => $price, // Precio Compra
				'cost' => $cost, // Precio de venta
				'porcentaje'=>$porcentaje, // Porcentaje Articulo
	...
	public function edit_item($line, $description, $serialnumber, $quantity, $discount,$porcentaje, $discount_type, $price, $cost, $receiving_quantity)
	{
		$items = $this->get_cart();
		if(isset($items[$line]))
		{
